<template>
    <div id="dailydouble" v-if="isDailyDouble && !hasWager">
        ...
        <div id="dailyadmin" v-if="isAdmin">
            ...
            <button type="button" id="dailydouble" @click="submitWager">Start</button>
        </div>
    </div>
</template>
...
<script>
    ...
    export default {
        ...
        computed: {
            ...mapGetters(['isDailyDouble', 'hasWager', 'isAdmin']),
        },
        methods: {
            submitWager() {
                this.$store.commit('setWager', {
                    teamId: parseInt(this.team_number),
